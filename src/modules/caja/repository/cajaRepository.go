...
func CerrarCajaRepository(usuario *bson.ObjectID, ctx context.Context) error {
	collection := config.MongoDatabase.Collection(enum.Caja)
	fecha := utils.FechaHoraBolivia()
	_, err := collection.UpdateOne(ctx, bson.M{"usuario": usuario, "flag": enum.EstadoNuevo}, bson.M{"$set": bson.M{"estado": enum.Cerrado, "fechaCierre": fecha}})
	if err != nil {
		return err
	}
	return nil
